...
Don't bother using this on Windows as I can't be bothered to install [[http://aspell.net/win32/][Aspell]] for
Windows.
...
#+begin_src emacs-lisp :tangle (if (eq system-type 'windows-nt) "no" init-file)
  (use-package flyspell-mode
    ...
    :bind (:map flyspell-mode-map
                ...
                ("M-#" . flyspell-auto-correct-region)))
#+end_src
...
Needs ~rnix-lsp~ or ~nixd~ for ~eglot~
...
Like ~undo-tree~ but doesn't hijack, modify or potentially corrupt the default,
idiosyncratic, but very powerful built-in undo logic.

However, sometimes it's easy to get lost all the undo/redo forks Emacs makes
available, so a visual indication is fantastic.
