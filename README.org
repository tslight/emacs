...
A fairly standard [[https://en.wikipedia.org/wiki/Literate_programming][literate]] Emacs configuration using [[https://orgmode.org/][Org mode]]. *Emacs 27+ ONLY*.
...
#+begin_src emacs-lisp :tangle (if (version< emacs-version "28") "no" init-file)
  (with-eval-after-load 'isearch-mode
    ...
    (repeat-mode)
    (message "Lazy loaded repeat-mode :-)"))
...
Goes in ~~/.emacs.d/early-init.el~:

#+begin_src emacs-lisp early-init
...
#+begin_src emacs-lisp
  (use-package orderless
    ...
    (setq completion-styles '(orderless initials flex partial-completion substring basic)
