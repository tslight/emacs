...
A fairly standard, vanilla & lightweight [[https://en.wikipedia.org/wiki/Literate_programming][literate]] *Emacs* configuration using
[[https://orgmode.org/][Org mode]] & [[https://github.com/jwiegley/use-package#readme][Use Package]].
...
that file into =~/.emacs.d/init.elc=, and then evaluate the resulting bytecode.

Be sure to back up your current =~/.emacs.d= with something like =mv
~/.emacs.d{,.bak}=, if there's anything you value in it.
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "C-x k" 'ts-kill-this-buffer)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "M-s M-s" 'ts-surround)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "C-c f r" 'ts-rename-this-file-and-buffer)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "C-M-/" 'ts-hippie-expand-completing-read)
#+end_src
...
to the tile above (or press =C-c ;= whilst under this heading), re-tangle and
re-load the file, by simply saving it.
